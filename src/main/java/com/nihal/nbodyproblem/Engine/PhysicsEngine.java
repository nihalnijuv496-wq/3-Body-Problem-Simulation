...
public class PhysicsEngine {
    ...
    {
        updateVerletWithAdaptiveTimeStepUpdate(bodies, Constants.timeStep);
    }
    ...
    public void updateVerletWithAdaptiveTimeStepUpdate(List<Body> bodies, double adaptiveTimeStep)
    {
        ...
        for (Body body: bodies)
        {
            ...
            for (Body bodyIter: bodies)
            {
                ...
                double scalingFactorAcc = Constants.G * bodyIter.getMass() / (softenedDistance*softenedDistance*softenedDistance);
    ...
    public void initializeAccelerations(List<Body> bodies) {
        for (Body body : bodies) {
            ...
            for (Body other : bodies) {
                ...
                double distance = displacement.magn();
                double softenedDistance = Math.sqrt(distance * distance + Constants.epsilon * Constants.epsilon);
                double scalingFactorAcc = Constants.G * other.getMass() / (softenedDistance*softenedDistance*softenedDistance);
